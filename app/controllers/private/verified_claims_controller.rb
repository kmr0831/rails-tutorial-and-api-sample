class Private::VerifiedClaimsController < ApplicationController
  ...
  def update
      ...
      if @ekyc_user.verified_claim
        ...
        render json: claim_json.merge(process_json)
      else
    ...
    private
      ...
      def claim_json
        ActiveModelSerializers::SerializableResource.new(
          @ekyc_user.verified_claim, serializer: VerifiedClaimSerializer, adapter: :json
        ).serializable_hash
      end
    
      def process_json
        ActiveModelSerializers::SerializableResource.new(
          @ekyc_user.verified_claim.verification_process, each_serializer: VerificationProcessSerializer, adapter: :json
        ).serializable_hash
      end
end
